// Automatically generated rust module for 'ims_send.proto' file
...
impl MessageWrite for FcitxEvent {
    fn get_size(&self) -> usize {
        0
        + if self.event == ims_send::KeyEvent::NUM_0 { 0 } else { 1 + sizeof_varint(*(&self.event) as u64) }
    }

    fn write_message<W: WriterBackend>(&self, w: &mut Writer<W>) -> Result<()> {
        if self.event != ims_send::KeyEvent::NUM_0 { w.write_with_tag(8, |w| w.write_enum(*&self.event as i32))?; }
        Ok(())
    }
}
